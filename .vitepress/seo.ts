...
const SITE_NAME = "WorkBuddy 航海图";
const SITE_ALTERNATE_NAME = "WorkBuddy Hangtu";
const ORGANIZATION_NAME = "WorkBuddy 航海图贡献者";
const GITHUB_URL = "https://github.com/mengxiangrui1211/workbuddy-hangtu";
const DEFAULT_DESCRIPTION =
  "WorkBuddy 从零到精通原创实战教程：安装上手、真实案例、进阶 Skill 与多 Agent、岗位与行业落地。";
...
function breadcrumbName(segment: string): string {
  ...
  const labels: Record<string, string> = {
    ...
    cases: "社区案例集",
    community: "社区共创",
    "reading-guide": "WorkBuddy 阅读指南",
    contribute: "参与共创",
    showcase: "案例墙",
    "difficulty-map": "航线图 · 难度地图",
  };
...
export function createSeoHead(
  ...
  const socialImageUrl = new URL("/og/workbuddy-hangtu.png", `${siteUrl}/`).href;
  ...
  const head: HeadConfig[] = [
    ...
    ["meta", { property: "og:image:alt", content: "WorkBuddy 航海图首页预览" }],
    ["meta", { name: "twitter:card", content: "summary_large_image" }],
    ["meta", { name: "twitter:title", content: title }],
    ["meta", { name: "twitter:description", content: description }],
    ["meta", { name: "twitter:image", content: socialImageUrl }],
    ["meta", { name: "twitter:image:alt", content: "WorkBuddy 航海图首页预览" }],
    [
      "script",
